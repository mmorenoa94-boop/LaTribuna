...
export async function POST(req: Request) {
  try {
    ...
    // Rate limit: 30 answers per minute per user
    const rl = await rateLimit(`answers:${session.user.id}`, 30, 60)
    if (!rl.success) {
      return NextResponse.json({ error: 'Demasiadas respuestas. Intenta en unos segundos.' }, { status: 429 })
    }
