...
import type { MatchRow, QuestionRow } from './types'
...
export function MatchesTab({ leagueId, initialMatches, initialQuestions, initialMatchId }: Props) {
  ...
  function handleQuestionsGenerated(newQuestions: QuestionRow[]) {
    ...
    setMatches((prev) =>
      prev.map((m) =>
        m.id === selectedMatchId ? { ...m, questionCount: newQuestions.length } : m
      )
    )
  }
  ...
  async function handleSaveScore(matchId: string, homeScore: number, awayScore: number, markFinished: boolean) {
    ...
    try {
      const res = await fetch(`/api/leagues/${leagueId}/admin/matches/${matchId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ homeScore, awayScore, ...(markFinished ? { status: 'FINISHED' } : {}) }),
      })
      if (res.ok) {
        setMatches((prev) =>
          prev.map((m) =>
            m.id === matchId ? { ...m, homeScore, awayScore, ...(markFinished ? { status: 'FINISHED' } : {}) } : m
          )
        )
      }
    } finally {
  ...
  const STATUS_COLORS: Record<string, string> = {
    SCHEDULED: 'text-lt-muted2', LIVE: 'text-lt-red', HALFTIME: 'text-lt-amber',
    FINISHED: 'text-lt-muted', CANCELLED: 'text-lt-red',
  }
            ...
            <div className="flex flex-col gap-4">
              ...
              {matches.length === 0 ? (
                <div className="bg-lt-card rounded-card border border-[rgba(255,255,255,0.07)] p-8 text-center">
                  ...
                  <p className="font-condensed text-sm text-lt-muted2 mt-1">Agrega el primer partido para empezar a crear preguntas</p>
                </div>
              ) : (
                <div className="flex flex-col gap-3">
                  {matches.map((match) => (
                    <motion.div key={match.id} className="bg-lt-card rounded-card border border-[rgba(255,255,255,0.07)] p-4 hover:border-lt-green/30 transition-colors w-full">
                      {confirmDeleteMatch === match.id ? (
                        <div className="flex flex-col items-center gap-3 py-2">
                          ...
                          <div className="flex gap-3">
                            <button onClick={() => setConfirmDeleteMatch(null)} className="px-4 py-2 rounded-btn bg-lt-card2 text-lt-muted2 font-condensed text-sm font-700">Cancelar</button>
                            <button onClick={() => handleDeleteMatch(match.id)} disabled={deletingMatchId === match.id}
                        ...
                        <>
                          <button onClick={() => selectMatch(match.id)} className="w-full text-left">
                            ...
                            <div className="flex items-center gap-3 flex-wrap">
                              ...
                              <span className={cn('font-condensed text-xs font-700', STATUS_COLORS[match.status] ?? 'text-lt-muted2')}>
                                {match.status === 'SCHEDULED' ? 'Programado' : match.status === 'LIVE' ? 'En vivo' : match.status === 'FINISHED' ? 'Finalizado' : match.status}
                              </span>
                            </div>
                            <div className="mt-2.5 flex items-center gap-1.5">
                              <span className="font-condensed text-xs text-lt-green font-700">{match.questionCount} pregunta{match.questionCount !== 1 ? 's' : ''}</span>
                              {match.questionCount === 0 && <span className="font-condensed text-xs text-lt-muted2">— toca para agregar</span>}
                            </div>
                          </button>
                          <div className="mt-3 pt-3 border-t border-[rgba(255,255,255,0.07)] flex gap-2">
                            ...
                            <button onClick={() => setConfirmDeleteMatch(match.id)}
          ...
          >
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-3">
                <button onClick={() => setSelectedMatchId(null)} className="text-lt-muted2 hover:text-lt-white transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-condensed text-xs text-lt-muted2">{selectedMatch && formatKickoff(selectedMatch.kickoffAt)}</p>
                </div>
                <div className="flex gap-1.5 flex-shrink-0">
                  <button onClick={() => setShowNotify(true)}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-btn bg-lt-blue/15 border border-lt-blue/40 text-lt-blue font-condensed text-sm font-700 active:scale-95 transition-all" title="Notificaciones">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                      <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" /><path d="M13.73 21a2 2 0 0 1-3.46 0" />
                    </svg>
                    <span className="hidden sm:inline">Notificar</span>
                  </button>
                  <button onClick={() => setShowGenerateQuestions(true)}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-btn bg-lt-amber/15 border border-lt-amber/40 text-lt-amber font-condensed text-sm font-700 active:scale-95 transition-all" title="Generar preguntas">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                      <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" />
                    </svg>
                    Generar
                  </button>
                  <button onClick={() => { setEditQuestion(null); setShowAddQuestion(true) }}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-btn bg-lt-green text-lt-black font-condensed text-sm font-700 active:scale-95 transition-all">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
                      <line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" />
                    </svg>
              ...
              {selectedMatch && (
                <ScoreInput match={selectedMatch} saving={savingScore}
              ...
              {(pendingCount > 0 || openCount > 0) && (
                <div className="flex gap-2">
                  {pendingCount > 0 && (
                    <button onClick={handleOpenAllPending} disabled={openingAll}
                  ...
                  {openCount > 0 && (
                    <button onClick={handleCloseAllOpen} disabled={closingAll}
              ...
              {loadingQs ? (
                ...
                <div className="bg-lt-card rounded-card border border-[rgba(255,255,255,0.07)] p-8 text-center">
                  ...
                  <p className="font-condensed text-sm text-lt-muted2 mt-1">Genera preguntas desde una plantilla o créalas manualmente</p>
                ...
                <div className="flex flex-col gap-3">
                  <AnimatePresence>
                    {questions.map((q) => (
                      <QuestionItem key={q.id} question={q} leagueId={leagueId}
                        onUpdated={handleQuestionUpdated} onDeleted={handleQuestionDeleted} onEdit={handleEdit} />
                    ))}
                  </AnimatePresence>
                </div>
              )}
      ...
      {selectedMatch && (
        <>
          <NotifyModal
            ...
            matchLabel={`${selectedMatch.homeTeam} vs ${selectedMatch.awayTeam}`}
            open={showNotify}
  ...
  return (
    <div className="bg-lt-card rounded-card border border-[rgba(255,255,255,0.07)] p-4">
      ...
      <div className="flex items-center gap-3 justify-center">
        <span className="font-condensed text-sm font-700 text-lt-white text-right flex-1 truncate">{match.homeTeam}</span>
        <input type="text" inputMode="numeric" pattern="[0-9]*" value={homeScore}
          onChange={(e) => { const v = e.target.value.replace(/\D/g, ''); setHomeScore(v === '' ? 0 : Math.min(99, Number(v))) }}
        ...
        <input type="text" inputMode="numeric" pattern="[0-9]*" value={awayScore}
          onChange={(e) => { const v = e.target.value.replace(/\D/g, ''); setAwayScore(v === '' ? 0 : Math.min(99, Number(v))) }}
          className="w-14 text-center font-bebas text-2xl text-lt-white bg-lt-card2 border border-[rgba(255,255,255,0.15)] rounded-btn py-1 focus:border-lt-green outline-none [appearance:textfield]" />
        <span className="font-condensed text-sm font-700 text-lt-white flex-1 truncate">{match.awayTeam}</span>
      </div>
      <div className="flex gap-2 mt-3">
        <button onClick={() => onSave(homeScore, awayScore, false)} disabled={saving}
        ...
        {!isFinished && (
          <button onClick={() => { if (confirm(`¿Finalizar ${match.homeTeam} ${homeScore} - ${awayScore} ${match.awayTeam}?`)) onSave(homeScore, awayScore, true) }}
            disabled={saving}
